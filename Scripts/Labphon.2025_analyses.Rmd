---
title: "Visualizations and analyses for LabPhon 20 abstract"
author: "Rachel"
date: \today
geometry: margin = 2cm
fontsize: 10pt
...
## Import 
```{r data_import}
data <- read_csv("C:/Users/rache/SI.SP/data/exp.1_data.csv", show_col_types = FALSE) %>%
  view()
```
...
```{r fig.cap="Participants' categorization responses in the 2AFC task during test.", fig.height=6, fig.width=12, warning=FALSE}
...
df <- 
  ...
  mutate(
    ...
    X = case_when(
      Condition_Attended.Label == "?s" ~ "/s/",
      Condition_Attended.Label == "?sh" ~ "/ʃ/",
      T ~ "fuck"),
    Talker_Bias =
      paste0(X, "-biased"
      ))
