...
for (f in file.list) {
  ...
  List <- read_csv(f, show_col_types = FALSE) %>%
    ...
    group_map(.f = ~ write_csv(.x, file = paste0(sub(".csv", "", f), "-Order10", "-Block", .y$block, ".csv")))
}
```
_____
...
select(c(name, notes, experiment_URL, participants)) %>%
...
mutate(
  completion_URL = paste0("https://app.prolific.co/submissions/complete?cc=CIGXKI5C")
) %>%
...
select(c(name, notes, completion_URL, experiment_URL, participants)) %>%

write_json(path = paste0("C:/Users/Rachel/Documents/SI.SP/Test_experiment-A.config"), pretty = TRUE)
```
