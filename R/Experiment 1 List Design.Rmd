...
# Code 9.05
```{r List.Design}
### Table with the information on how the stims should be paired + voice x accent x ear assignment

tibble(
  AttendedCategory = c(
  ## This will be Materials A
    "sh", 
    "sh", 
    "sh", 
    "sh", 
...
mutate(
  ...
  ## M/F Voice
    case_when(
      Assignment_TalkerToGender == 
        "Attended: Female (F)" ~ ".F", T ~ ".M"),
    
  ## L/R Ear
    case_when(
      Assignment_TalkerToEar == 
        "Attended: Left (L)" ~ "-L", T ~ "-R")
  ))%>% 
