...
## Group files
group_by(Shift, Talker.A_Gender, Talker.A_Ear) %>%

group_map(.f = ~ write_csv(.x, file = paste0("C:Stimuli/Experiment_1/Lists/Exposure_Phase/Critical_Lists/Critical_Talker.A_", .y$Talker.A_Gender, "_", .y$Talker.A_Ear, "_", .y$Shift, ".csv")
))
```
...
tibble(
 ...
 mutate(
  ...
  mutate(
      Set.Version =
        case_when( 
           Talker.A_Gender == "Male (M)" &
           Talker.A_Ear == "Right (R)"
            ~ "1", 
...
group_map(.f = ~ write_csv(.x, file = paste0("C:Stimuli/Experiment_1/Lists/Exposure_Phase/Filler_Lists/Filler.", .y$Set.Combo, ".csv")
))
```
