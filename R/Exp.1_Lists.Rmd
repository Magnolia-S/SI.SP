...
## Adds in column noting Gender + Ear Assignment for Talker B
 ...
 mutate(
  ...
  ## Add column for shift --> labeling the list
  mutate(
    Is.Shifted =
      case_when(
        Shift == "Shifted"
...
## Group files
group_by(Shift, Filler.Match, Talker.A_G, Talker.A_E) %>%

group_map(.f = ~ write_csv(.x, file = paste0("C:Stimuli/Experiment_1/Lists/Exposure_Phase/Critical_Lists/Critical.",  .y$Filler.Match,"_", .y$Shift, "_Talker.A_", .y$Talker.A_G, "_", .y$Talker.A_E, ".csv")
))
```
...
### I want to rename the files and copy them into this folder: C:\Users\Rachel\Documents\Social.Influence_Speech.Perception\SI.SP\R\Stimuli\Experiment_1\Items_Critical

```{r Critical Rename_S.F.L}
## Left: Female S Unambiguous; Right: Male Sh Unambiguous
for (f in list.files(path = "Stimuli/Experiment_1/Items_Critical/F.S_M.Sh/Nonambiguous")) {
f.Category = gsub("^200ms", "Critical", f)
  ...
  write(f.right_Gender.Ear, "Stimuli/Experiment_1/Items_Critical")
}
```

```{r Critical Rename_As.F.L}
for (f in list.files(path = "Stimuli/Experiment_1/Items_Critical/F.S_M.Sh/Ambiguous/")) {
  f.Category = gsub("^200ms", "Critical", f)
  f.left_Shift = gsub("F.Xs", "As", f.Category)
  f.left_Gender.Ear = gsub("_M.", ".F.L_", f.left_Shift)
  f.right_Shift = gsub("Xsh", "As", f.left_Gender.Ear)
  f.right_Gender.Ear = gsub(".wav", ".M.R.wav", f.right_Shift)
  
  print_csv(f.right_Gender.Ear)
}
```

```{r Critical Rename_Sh.F.L}
for (f in list.files(path = "Stimuli/Experiment_1/Items_Critical/F.Sh_M.S/Nonambiguous/")) {
  f.Category = gsub("^200ms", "Critical", f)
  ...
  print(f.right_Gender.Ear)
}
```

```{r Critical Rename_Ash.F.L}
for (f in list.files(path = "Stimuli/Experiment_1/Items_Critical/F.Sh_M.S/ambiguous/")) {
  f.Category = gsub("^200ms", "Critical", f)
...
for (f in list.files(path = "Stimuli/Experiment_1/Items_Critical/M.S_F.Sh/Nonambiguous/")) {
  f.Category = gsub("^200ms", "Critical", f)
  f.left_Shift = gsub("M.S", "Ash", f.Category)
  f.left_Gender.Ear = gsub("_F.", ".M.L_", f.left_Shift)
  f.right_Shift = gsub("Sh", "", f.left_Gender.Ear)
...
for (f in list.files(path = "Stimuli/Experiment_1/Items_Critical/M.S_F.Sh/Ambiguous/")) {
  f.Category = gsub("^200ms", "Critical", f)
...
for (f in list.files(path = "Stimuli/Experiment_1/Items_Critical/M.Sh_F.S/Nonambiguous/")) {
  f.Category = gsub("^200ms", "Critical", f)
